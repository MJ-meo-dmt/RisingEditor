...
class FileBrowser(DirectObject):
    ...
    def createElementDir(self, element, data, filepath):
        ...
        entry = self.base.editorGui.CreateElement(element)
        entry.SetAttribute("id", data)
        entry.AddEventListener('click', lambda:self.openFolderDir(data, filepath), True)
        entry2 = self.base.editorGui.CreateElement('br')
    ...
    def openFolderDir(self, path, filepath):
        self.path = path
        print self.path, "Folder Dir"
        #del self.tempDirView
        fileview = self.base.editorGui.GetElementById("filepathlist")
        for child in fileview.child_nodes:
            print child, "this is one"
            fileview.RemoveChild(child)
            del child
